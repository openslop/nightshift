...
10のジョブを、順番にひとつずつ実行します。セキュリティ、バグ、読みやすさ、保守性、パフォーマンス、規約、アーキテクチャ、スモークテスト、issue。それぞれのジョブが、エージェントをまるごと独り占めして、ほかのことはしません。
...
| `readability`          | 意味の伝わらない名前。二度読まないとわからないコード。行は減らし、増やしません。   |
